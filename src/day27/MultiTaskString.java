...
public class MultiTaskString {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < numbers.length; i++) { //check each index of two d a
            //    System.out.println(Arrays.toString(numbers[i]));
            for (int j = 0; j < numbers[i].length; j++) { //check each elements of single da
        ...
        for (int i = 0; i < ages.length; i++) {
            for (int j = 0; j < ages[i].length; j++) {
                // how many time loop execute each element ages [1] >> 5 times
                // ages [0] >> 1 times
                // ages [2] >> 5times
                //
                if (ages[i][j] % 2 != 0) {
